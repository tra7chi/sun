...
其他是 cookie 需要的参数
当程序本身用到session时，建议将此php购物车类改为cookie方式实现。
...
var $cookiedomain = '';
	...
	function __construct($cartname = 'myCart', $session_id = '', $savetype = 'cookie', $cookietime = 86400, $cookiepath = '/', $cookiedomain = '') {
	// 采用 session 存储
		if ($savetype == 'session') {
	...
	function save() {
		...
		elseif ($this->savetype == 'cookie') {
			if (isset($this->data))
				...
				setcookie($this->cartname, "1", $this->cookietime + (time()-86500), $this->cookiepath, $this->cookiedomain);
		}
	}
